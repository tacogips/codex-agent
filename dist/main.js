...
function groupFilePath(configDir) {
  return join8(resolveConfigDir(configDir), GROUPS_FILE);
}
...
import { readFile as readFile5, writeFile as writeFile3, mkdir as mkdir2, rename as rename2 } from "fs/promises";
...
function queueFilePath(configDir) {
  return join9(resolveConfigDir2(configDir), QUEUES_FILE);
}
...
function bookmarkFilePath(configDir) {
  return join10(resolveConfigDir3(configDir), BOOKMARKS_FILE);
}
...
import { mkdir as mkdir4, readFile as readFile7, rename as rename4, writeFile as writeFile5 } from "fs/promises";
...
function tokenFilePath(configDir) {
  return join11(resolveConfigDir4(configDir), TOKENS_FILE);
}
...
function fileIndexPath(configDir) {
  return join12(resolveConfigDir5(configDir), FILE_INDEX_FILE);
}
